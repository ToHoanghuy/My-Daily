...
import {uploadMonthIncome} from '../../../../ReduxToolKit/Slices/StatisticSlice';
import {useEffect} from 'react';
import {QUARTER_TXT} from '../../../Assets/Data/Months';
...
const Income = ({
  ...
  useEffect(() => {
    ...
    if (typeStatistic === 'Quarter' || typeStatistic === 'Month') {
      tmp = QUARTER.map(item => item.income);
    }
    if (typeStatistic === 'Customize') {
      tmp = CUSTOMIZE.map(item => item.income);
    }
  ...
  const listStatistics = useSelector(state => state.Statistics).monthIncome;
  ...
  const onLinePress = press => {
    ...
    getDataMonthIncome(uid, yearPress, press.month, WALLET).then(
      statisticMonth => {
        dispatch(uploadMonthIncome(statisticMonth));
    ...
    return (
      <View key={index} style={styles.item}>
        ...
        <Text style={styles.item_name} numberOfLines={1}>
          {langugage === 'English' ? statistic.name.EN : statistic.name.VN}
        </Text>
        {typeStatistic !== 'Year' && (
          <Text style={styles.item_percent}>
            {+((statistic.value / sumStatistics(STATISTIC)) * 100).toFixed(2)} %
          </Text>
        )}
      ...
      <View key={index} style={styles.item}>
        ...
        {typeStatistic !== 'Year' && (
          <Text
            style={[
              styles.item_percent,
              {color: typePress === 2 ? ColorCustom.blue : ColorCustom.orange},
            ]}>
            {+((statistic.value / sumStatistics(STATISTIC)) * 100).toFixed(2)} %
          </Text>
        )}
    ...
    return (
      <View style={{width}}>
        <LineChart
          ...
          langugage={langugage}
          label_xAxis={langugage === 'English' ? 'Month' : 'Tháng'}
    ...
    <View style={{width}}>
      ...
      <View
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={[styles.btn_txt, {color: ColorCustom.gray}]}>
              {langugage === 'English' ? "Don't have data" : 'Không có dữ liệu'}
            </Text>
          }
        />
      </View>
